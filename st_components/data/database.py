...
from st_components.data.models import Conversation, Chat
...
def save_chat(chat):
    with create_connection() as cursor:
        cursor.execute("INSERT INTO chats (id, conversation_id, role, content) VALUES (?, ?, ?, ?)",
                       (str(uuid.uuid4()), chat.conversation_id, chat.role, chat.content))
