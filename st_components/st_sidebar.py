...
def set_open_ai_server_credentials():
    with st.expander(label="Settings", expanded=(not st.session_state['chat_ready'])):
        openai_api_base = st.text_input('OpenAI API Base:', type="default", value="http://192.168.31.15:8000/v1")
        openai_api_key = st.text_input('OpenAI API Key:', type="password", value="fake_key")
        os.environ['OPENAI_API_BASE'] = openai_api_base
        os.environ['OPENAI_API_KEY'] = openai_api_key
        model = st.selectbox(
            label='🔌 models',
...
def set_open_router_server_credentials():
    with st.expander(label="Settings", expanded=(not st.session_state['chat_ready'])):
        ...
        button_container = st.empty()
        save_button = button_container.button("保存 🚀", key='open_router_save_model_configs')
